...
SCREAMING_SNAKE_CASE : re.Pattern = re.compile("^[A-Z_][A-Z_0-9]*$")
snake_case : re.Pattern= re.compile("^[a-z_][a-z_0-9]*$")
...
snake_killer : re.Pattern= re.compile(r"([^_]+)")
...
def snake2camel(string : str):
	return "".join(word.capitalize() if i > 0 else word for i, word in enumerate(snake_killer.findall(string.lower())))

def snake2pascal(string : str):
	return "".join(map(*this.capitalize(), snake_killer.findall(string.lower())))
