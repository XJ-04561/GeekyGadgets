...
class CSS(ABC):
	pass
	# def __str__(cls):
	# 	return _Case.KebabCase(cls.__name__ if isinstance(cls, type) else cls.__class__.__name__)
	# def __hash__(cls):
	# 	return hash(str(cls))
	# def __eq__(cls, other):
	# 	return hash(cls) == hash(other) if isinstance(other, Hashable) else NotImplemented
CSS.register(AttributeCSS)

class CascadingStyleSheet(Ruleset, CSS):
	def __init__(self, selector : str, properties : "dict[str,str|Number|ValueStatement]") -> None:
		dict.update(self, properties)

VALUE_STATEMENT_PATTERN : re.Pattern = re.compile(r"(\d*[.]?\d*|calc[(](?:[(].*?[)]|.)*?[)]|(?<=\b)(?!calc)\w+|!\w+\s*;?$|\s+)+")
VALUE_FUNCTION_PATTERN : re.Pattern = re.compile(r"(\w+)[(](.*)[)]")
VALUE_UNIT_PATTERN : re.Pattern = re.compile(r"(\d+[.]?\d*|\d*[.]?\d+)(.+)?")
def evaluateValueStatement(string : str):
	# if not VALUE_STATEMENT_PATTERN.fullmatch(string):
	# 	return ValueStatement()
	if m := re.search(r"!\s*([-\w]+)\s*[;]?\s*$", string):
		string = string.removesuffix(annotation := m.group(1))
	else:
	...
	for i, c1 in iterator:
		if c1.isspace():
			continue
		level = 0
		quote = False
		for j, c2 in iterator:
			if not level and not quote and c2.isspace():
				break
			elif c2 == "\"":
				if level:
					quote = not quote
				elif quote:
					content.append(string[i:j+1])
					break
			elif c2 == "(" and not quote:
				level += 1
			elif c2 == ")" and not quote:
				level -= 1
		if m := VALUE_FUNCTION_PATTERN.fullmatch(string[i:j]):
			name, args = m.groups()
			if name not in _MultiValue.SUBCLASSES:
				_MultiValue.__class__(_Case.PascalCase(name), (_MathableValue,_MultiValue), {})
			content.append(_MultiValue.SUBCLASSES[name].parse(args))
		elif m := VALUE_UNIT_PATTERN.fullmatch(string[i:j]):
			value, unit = m.groups()
			value = _Functions.parseNum(value)
			if unit is None:
				content.append(value)
			else:
				content.append(ValueUnit(value, unit))
		else:
			content.append(string[i:j])
	if len(content) == 1:
		return content[0]
	else:
		return ValueStatement(*content)

class _MathableValue(ABC):
	
	content : "tuple[_MathableValue]"
	
	def __repr__(self) -> str:
		return f"<{self.__class__.__name__} object '{self!s}' at {id(self):#x}>"

	def __mul__(self, other : "_MathableValue") -> "Calc":
		if other is None:
			return self
		if (isinstance(other, ValueUnit) or isinstance(other, Calc) and any(e in "+-" for e in other.content)) \
			and (isinstance(other, ValueUnit) or isinstance(self, Calc) and any(e in "+-" for e in self.content)):
			return Calc(*self.content, "*", *other.content)
		else:
			return Calc(self, "*", other)
	def __rmul__(self, other : "_MathableValue") -> "Calc":
		if other is None:
			return self
		if (isinstance(other, ValueUnit) or isinstance(other, Calc) and any(e in "+-" for e in other.content)) \
			and (isinstance(other, ValueUnit) or isinstance(self, Calc) and any(e in "+-" for e in self.content)):
			return Calc(*other.content, "*", *self.content)
		else:
			return Calc(other, "*", self)
	def __truediv__(self, other : "_MathableValue") -> "Calc":
		if other is None:
			return self
		if (isinstance(other, ValueUnit) or isinstance(other, Calc) and any(e in "+-" for e in other.content)) \
			and (isinstance(other, ValueUnit) or isinstance(self, Calc) and any(e in "+-" for e in self.content)):
			...
			return Calc(self, "/", other)
	def __rtruediv__(self, other : "_MathableValue") -> "Calc":
		if other is None:
			return self
		if (isinstance(other, ValueUnit) or isinstance(other, Calc) and any(e in "+-" for e in other.content)) \
			and (isinstance(other, ValueUnit) or isinstance(self, Calc) and any(e in "+-" for e in self.content)):
			...
			return Calc(other, "/", self)
	def __add__(self, other : "ValueUnit") -> "Calc":
		if other is None:
			return self
		return Calc(self, "+", other) if not isinstance(other, Calc) else Calc(*self.content, "+", *other.content)
	def __radd__(self, other : "ValueUnit") -> "Calc":
		if other is None:
			return self
		return Calc(other, "+", self) if not isinstance(other, Calc) else Calc(*other.content, "+", *self.content)
	def __sub__	(self, other : "ValueUnit") -> "Calc":
		if other is None:
			return self
		return Calc(self, "-", other) if not isinstance(other, Calc) else Calc(*self.content, "-", other)
	def __rsub__	(self, other : "ValueUnit") -> "Calc":
		if other is None:
			return self
		return Calc(other, "-", self) if not isinstance(other, Calc) else Calc(*other.content, "-", self)
	def __abs__(self):
		return ValueUnit(abs(self.value), self.unit) if isinstance(self, ValueUnit) else Abs(self)
	def __neg__(self):
		return ValueUnit(-self.value, self.unit) if isinstance(self, ValueUnit) else Calc("-", self)
	def __pos__(self):
		return ValueUnit(+self.value, self.unit) if isinstance(self, ValueUnit) else Calc("+", self)
	
class _MultiValue(ABC):
	
	NAME : _Case.CamelCase
	SEP : str = " "
	PATTERN : re.Pattern = re.compile(r"""(\w*[(]|[)]|[\w-]+|(?<![^\w])(?:\d+[.]?\d|\d*[.]?\d+)\S*|["][^"]*?(?<![\\])["]|['][^']*?(?<![\\])[']|\s+|.+?)+""")
	SUBCLASSES : "dict[str,type[_MultiValue]]" = {}

	def __init_subclass__(cls) -> _Case.NoneType:
		cls.NAME = _Case.CamelCase(cls.__name__)
		cls.SUBCLASSES[cls.NAME] = cls
		return super().__init_subclass__()

	@classmethod
	def parse(cls, string : str):
		if not cls.PATTERN.fullmatch(string):
			return None
		iterator = iter(filter(re.compile(r"\S").match, cls.PATTERN.split(string)))

		content = []
		for word in iterator:
			if word == "(":
				nesting = 0
				name, *_ = word.partition("(")
				parts = []
				for innerWord in iterator:
					if innerWord.startswith(")"):
						nesting -= 1
					if nesting < 0:
						valueCls = _MultiValue.SUBCLASSES.get(_Case.CamelCase(name), Calc)
						content.append(valueCls.parse(" ".join(parts)))
						break
					parts.append(innerWord)
					if innerWord.endswith("("):
						nesting += 1
			elif word.startswith("\"") and word.endswith("\""):
				content.append(word)
			elif word.startswith("'") and word.endswith("'"):
				content.append(word)
			elif Calc.OPERATOR_PATTERN.fullmatch(word):
				content.append(word.strip())
			else:
				content.append(ValueUnit(word))
		return cls(*content)
	
	def __repr__(self) -> str:
		return f"<{self.__class__.__name__} object '{self!s}' at {id(self):#x}>"
	def __str__(self):
		return f"{self.NAME}({self.SEP.join(map(str,self.content))})"

class ValueUnit(_MathableValue):
	
	PATTERN  : re.Pattern = re.compile(r"(\d+[.]?\d|\d*[.]?\d+)([\D]?\S*)")

	value : Number
	unit : str
	content : "tuple[ValueUnit]" = property(lambda self: (self,))

	def __init__(self, value : Number|str, unit : str="") -> _Case.NoneType:
		if not isinstance(value, Number):
			self.value = _Functions.parseNum(value)
		else:
			self.value = value
		self.unit = unit or ""
	...
	@classmethod
	def parse(cls, string : str|Number):
...
class Calc(_MathableValue, _MultiValue):
	
	OPERATOR_PATTERN : re.Pattern = re.compile(r"\s*([-+*/])\s*")

	content : "tuple[ValueUnit|Number|str|Calc]"

	def __init__(self, *valuesAndOperators : str) -> None:
		self.content = valuesAndOperators

class Abs(_MathableValue, _MultiValue):
...
class Var(_MathableValue, _MultiValue):
	
	name : _Case.CamelCase
	content : tuple[str] = property(lambda self: f"--{_Case.KebabCase(self.name).upper()}")

	def __init__(self, name : str) -> None:
		self.name = _Case.CamelCase(name.strip().partition("--")[-1])
	
	@classmethod
	def parse(cls, string : str):
		return cls(string)

class ValueStatement(_MultiValue):
	
	annotation : str|None
	"""Stuff like: ... !important"""
	content : tuple[ValueUnit|Calc|str|Number]

	def __init__(self, *content, annotation=None) -> _Case.NoneType:
	...
	def __str__(self):
		return self.SEP.join(map(str, self.content)) + (f" !{self.annotation}")*bool(self.annotation)
...
# 	suffix = _Classy.ClassProperty(lambda self: _Case.KebabCase(self.__name__) if isinstance(self, type) else _Case.KebabCase(self.__class__.__name__))
...
# 	name = _Classy.CachedClassProperty(lambda self: _Case.KebabCase(self.__name__) if isinstance(self, type) else _Case.KebabCase(self.__class__.__name__))
# 	def __str__(self):
# 		return f"{_Case.KebabCase(self.__class__.__name__)}({', '.join(map(lambda x:str(x) if not isinstance(x, str) else SyntaxContext.repString(x), self))})"
...
# 	def __str__(cls):
# 		return _Case.KebabCase(cls.__name__)
# 	def __hash__(cls):
# 		return hash(str(cls))
# 	def __eq__(cls, other):
# 		return hash(cls) == hash(other) if isinstance(other, Hashable) else NotImplemented
...
# PROPERTIES : "_SpecialTypes.NameSpace[str,type[CssProperty]]" = _SpecialTypes.NameSpace(map(lambda prop: (prop.__name__, prop), CssProperty.__subclasses__()))
# UNITS = tuple(_Walkers.SubClassWalker(Unit))
